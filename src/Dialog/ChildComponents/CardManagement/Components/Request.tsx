...
export function Request():JSX.Element{
    const { game, myCards, userId, setActive} = useContext(GameContext)
    ...
    return <div>
        <h2>Request</h2>
        <span>Ha sido </span>
        <Dropdown className={styles.textfield} options={myCards.filter(x=>x.type === "Suspect").map(x=>{return {key: x.id, text: x.name}})} onChange={(e,option)=>setSuspect(option as IDropdownOption)} placeholder="Select a Suspect" selectedKey={suspect?.key} />
        <span>con</span>
        <Dropdown className={styles.textfield} options={myCards.filter(x=>x.type === "Weapon").map(x=>{return {key: x.id, text: x.name}})} onChange={(e,option)=>setWeapon(option as IDropdownOption)} placeholder="Select a Weapon" selectedKey={weapon?.key} />
        <span>en</span>
        <Dropdown className={styles.textfield} options={myCards.filter(x=>x.type === "Room").map(x=>{return {key: x.id, text: x.name}})} onChange={(e,option)=>setRoom(option as IDropdownOption)} placeholder="Select a Room" selectedKey={room?.key} />
